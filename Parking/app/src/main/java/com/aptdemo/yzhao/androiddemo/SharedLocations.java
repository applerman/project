 ...
 */
public class SharedLocations extends AppCompatActivity {

    private ListFragment mList;
    private String TAG  = "My History";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ArrayList<NamedLocation> locations_array = new ArrayList<NamedLocation>();
        final String request_url = "http://parkingrighthere.appspot.com/park?viewparking=true&shared_parking=true&user_email=" + Homepage.email;
        ...
        httpClient.get(request_url, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] response) {
                try {
                    ...
                    for (int i = 0; i < parkingLats.length(); i++) {
                        locations_array.add(new NamedLocation(parkingDatetimes.getString(i), new LatLng(
                                ...
                                Double.parseDouble(parkingLons.getString(i))
                        )));
                    }
                ...
                final NamedLocation[] LIST_LOCATIONS = new NamedLocation[locations_array.size()];
                for (int i = 0; i < locations_array.size(); i++)
                    LIST_LOCATIONS[i] = locations_array.get(i);

                // Set a custom list adapter for a list of locations
                mAdapter = new MapAdapter(context, LIST_LOCATIONS);
